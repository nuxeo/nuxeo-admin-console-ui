...
describe("NuxeoStreamProcessorInfoComponent", () => {
  ...
  beforeEach(() => {
    mockStreamService = {
      getStreamProcessorInfo: vi
        .fn()
        .mockName("StreamService.getStreamProcessorInfo"),
  ...
  it("should call loadJsonData on ngOnInit", () => {
    ...
    mockStreamService.getStreamProcessorInfo.mockReturnValue(
      of({ key: "value" })
    );
  ...
  it("should load JSON data successfully when streamProcessorJsonData is null", () => {
    ...
    mockStreamService.getStreamProcessorInfo.mockReturnValue(of(mockData));
  ...
  it("should handle error when loading JSON data fails and error object is present", () => {
    ...
    component.streamProcessorJsonData = null;
    vi.spyOn(component, "showErrorMessage");
    component.loadJsonData();
    expect(component.isDataLoaded).toBe(true);
  ...
  describe("isValidData", () => {
    ...
    it("should return true for object with multiple properties", () => {
      expect(component.isValidData({ prop1: "value1", prop2: "value2" })).toBe(
        true
      );
    });
  });
  ...
  it("should set isError to false when loading data successfully", () => {
    ...
    mockStreamService.getStreamProcessorInfo.mockReturnValue(of({}));
    component.loadJsonData();
    expect(component.isError).toBe(false);
  });
